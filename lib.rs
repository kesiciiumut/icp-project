...
struct CreateProposal{
 ...
 impl BoundedStorable for Storable {
...
thread_local! {
    ...
    RefCell:: new(StableBTreeMap::init(MEMORY_MANAGER.with(|m| m.barrow().get(MemoryId::new (0)))));

}

 #[ic_cdk::query]
 fn get_proposal(key: u64) -> Option<Proposal> {
  PROPOSAL_MAP.with(|p:| p.barrow().get(&key))
 }

 #[ic_cdk::query]
 fn get_proposal_count() -> u64 {
    PROPOSAL_MAP.with(|p:| p.barrow().len())
 }
 ...
 fn create_proposal(key: u64, proposal: CreateProposal) -> Option<Proposal> {
    ...
    PROPOSAL_MAP.with(|p:| p.barrow_mut().insert(key,value))
 }

 #[ic_cdk::update]
 fn edit_proposal(key: u64, proposal: CreateProposal) -> Result<(), VoteError> {
    PROPOSAL_MAP.with(|p: |{
        let old_proposal_opt=p.barrow().get(&key);
        ...
        if old_proposal.owner != ic_cdk :: caller(){
            return Err(VoteError::AccessRejeceted);
        ...
        let res = p.barrow_mut().insert(key,value);
        match res{
 ...
 fn end_proposal(key: u64) -> Result<(), VoteError> {
    PROPOSAL_MAP.with(|p: |){
        ...
        let res = p.barrow_mut().insert(key,old_proposal);
        match res{
 ...
 fn vote(key: u64, choice: Choice) -> Result<(), VoteError> {
    PROPOSAL_MAP.with(|p: |){
        let proposal_opt = p.barrow().get(&key);
        ...
        let res = p.barrow_mut().insert(key,proposal);;
